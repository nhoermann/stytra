...
class Parametrized(object):
    ...
    def as_hastraits(self):
        # Lazily imported: only this (Jupyter/traitlets-notebook) path needs
        # `traitlets`, so the rest of stytra's Qt-based parameter GUI never
        # pays for that dependency.
        from stytra.lightparam.param_traits import HasTraitsLinked
